...
class MinecraftServer:
    ...
    async def query(self):
        ...
        logger.info(
            f"Queried {self.name} ({self.address}) in {round((end - start), 2)} seconds"
        )
...
async def metric_collection(edition, servers):
    ...
    workers = [
        asyncio.create_task(collect_metrics(sem, edition, server)) for server in servers
    ]
...
class MinecraftCollector(object):
    ...
    def collect(self):
        gauge = GaugeMetricFamily(
            ...
            "Minecraft server online player counts",
            labels=[
                "server_edition",
                "server_name",
                "server_slug",
            ...
            try:
                ...
                for server in metrics:
                    if server.version is not None and server.playercount is not None:
                        ...
                        logger.warning(
                            f"{server} did not return any metrics, not adding to gauge"
                        )
                end = timer()
                logger.info(
                    f"Finished collecting {server.edition} metrics in {round((end - start), 2)} seconds"
                )
        yield gauge
